...
def get_posts():
    complete_posts = []
    # Caution in the code the ta instructed the below line to be = db(db.post.is_reply == None).select( but pycharm
...
def save_post():
    ...
    post_id = request.json.get('id') # Note: id can be none. it means we are creating a new post
    content = request.json.get('content')
    is_reply = request.json.get('is_reply')

    new_id = db.post.update_or_insert(          # if updating new id = None
        db.post.id == post_id,
        is_reply=is_reply,
        content=content
    )
